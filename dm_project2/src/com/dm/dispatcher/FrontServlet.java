...
public class FrontServlet extends HttpServlet {
	...
	private static final long serialVersionUID = -4794639507416889505L;
	...
	private void requestPro(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("[FrontServlet] command >>>>> "+command);
		...
		if(handlerMapper != null) {
			...
			if(act == null) {
				System.out.println("이런 액션 없다!! : " + command);
			}else {
				view = act.execute(request, response);
				System.out.println("[FrontServlet] view >>>>> " + view);
